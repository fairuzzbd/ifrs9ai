...
export default function ImpactPdHistoryPage() {
  const { id } = useParams<{ id: string }>();
  const perms = usePermissions();

  const [cursor, setCursor] = React.useState<string | null>(null);
  const [allEntries, setAllEntries] = React.useState<
    Awaited<ReturnType<typeof impactPdApi.getHistory>>["data"]
  >([]);

  const { data, isLoading } = useQuery({
    queryKey: ["impact-pd-history", id, cursor],
    queryFn: () => impactPdApi.getHistory(id, { limit: 50, cursor: cursor ?? undefined }),
    enabled: !!id,
  });
  ...
  return (
    <div className="container mx-auto py-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Riwayat Audit — Impact PD</h1>
        <Button variant="outline" size="sm" asChild>
          <Link href={`/master/impact-pd/${id}`}>&larr; Kembali ke Detail</Link>
        </Button>
      </div>
