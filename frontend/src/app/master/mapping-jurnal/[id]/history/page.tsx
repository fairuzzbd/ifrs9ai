...
export default function MappingJurnalHistoryPage() {
  const { id } = useParams<{ id: string }>();
  const perms = usePermissions();

  const [cursor, setCursor] = React.useState<string | null>(null);
  const [allEntries, setAllEntries] = React.useState<
    Awaited<ReturnType<typeof mappingJurnalApi.getHistory>>["data"]
  >([]);

  const { data, isLoading } = useQuery({
    queryKey: ["mapping-jurnal-history", id, cursor],
    queryFn: () =>
      mappingJurnalApi.getHistory(id, {
        limit: 50,
    ...
    <div className="container mx-auto py-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">
          Riwayat Audit — Mapping Jurnal
        </h1>
        <Button variant="outline" size="sm" asChild>
          <Link href={`/master/mapping-jurnal/${id}`}>
