...
import type { AuditHistoryEntry } from "@/lib/api/lgd-basel.api";

export default function LGDBaselHistoryPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["lgd-basel-history", id, cursor],
    queryFn: () =>
      lgdBaselApi.getHistory(id, {
        limit: 50,
    ...
    <div className="container mx-auto py-6 space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold">
          Riwayat Audit — LGD Pool
        </h1>
        <Button variant="outline" size="sm" asChild>
          <Link href={`/master/lgd-basel/${id}`}>&larr; Kembali ke Detail</Link>
        </Button>
      </div>
