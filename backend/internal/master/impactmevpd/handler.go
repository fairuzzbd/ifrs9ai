...
// Thin: parse → service → response. No business logic here.
type Handler struct {
	svc       *Service
...
// ─── GET /master/impact-mev-pd/export ─────────────────────────────────────────
...
func (h *Handler) Export(c *gin.Context) {
	...
	if format != "csv" {
		// XLSX not implemented per task spec; return 501.
		response.Error(c, domainerrors.New(domainerrors.CodeValidationFailed,
...
// ─── DELETE /master/impact-mev-pd/:id ─────────────────────────────────────────
...
// ─── GET /master/impact-mev-pd/:id/history ────────────────────────────────────
...
// Approve2 handles POST /api/v1/master/impact-mev-pd/:id/approve2.
...
// parseID parses the :id path parameter as UUID.
