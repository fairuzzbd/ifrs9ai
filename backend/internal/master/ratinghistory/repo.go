...
// SearchCols values are interpolated as "r.%s" into SQL search predicates; any column
...
func init() {
	...
	for _, c := range SearchCols {
		if _, ok := allowed[c]; !ok {
			panic(fmt.Sprintf("ratinghistory.SearchCols contains %q which is not in AllAllowedCols — potential SQLi vector", c))
		}
	}
}
