// Package instrumen — HTTP handler layer for mst.instrumen (APP-A-MSTR-011).
//
// Responsibilities: parse HTTP inputs → call service → return response envelopes.
...
// Handler is the HTTP handler for instrumen endpoints.
type Handler struct {
	svc       *Service
...
// ─── GET /master/instrumen ────────────────────────────────────────────────────

// List handles GET /api/v1/master/instrumen.
...
// ─── POST /master/instrumen ───────────────────────────────────────────────────

// Create handles POST /api/v1/master/instrumen.
...
// ─── GET /master/instrumen/export ─────────────────────────────────────────────

// Export handles GET /api/v1/master/instrumen/export.
func (h *Handler) Export(c *gin.Context) {
	...
	filename := fmt.Sprintf("instrumen-%s.csv", time.Now().Format("20060102"))
...
func (h *Handler) GetByID(c *gin.Context) {
	...
	m, err := h.svc.GetByID(c.Request.Context(), id, false)
	if err != nil {
		response.Error(c, err)
		return
	}
...
// ─── GET /master/instrumen/:id/history ────────────────────────────────────────
...
func parseUUIDParam(c *gin.Context, param string) (uuid.UUID, error) {
	...
	if err != nil {
		return uuid.Nil, domainerrors.New(domainerrors.CodeValidationFailed,
			fmt.Sprintf("Path parameter '%s' harus berupa UUID valid.", param),
			domainerrors.Detail{Field: "path." + param, Rule: "uuid", Message: "Bukan UUID valid"},
		)
	}
	return id, nil
}
