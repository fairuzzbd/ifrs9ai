...
// WorkflowHook implements workflow.EntityHook for mst.instrumen.
// BeforeCommit runs inside the workflow transaction to keep
// mst.instrumen.workflow_status in sync atomically.
...
// BeforeCommit syncs workflow_status on mst.instrumen inside the workflow tx.
func (h *WorkflowHook) BeforeCommit(ctx context.Context, tx *sql.Tx, evt workflow.HookEvent) error {
	...
	if err := h.repo.UpdateWorkflowStatus(ctx, tx, evt.EntityID, wfStatus, evt.ActorID); err != nil {
		return fmt.Errorf("instrumen.WorkflowHook.BeforeCommit: %w", err)
	}
	return nil
}
