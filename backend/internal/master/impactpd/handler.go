// Package impactpd — HTTP handler layer for mst.impact_pd (APP-A).
package impactpd
...
// GetActive handles GET /api/v1/master/impact-pd/active.
// Returns the APPROVED row for the given periode_id.
...
func (h *Handler) Submit(c *gin.Context) {
	...
	h.delegateWorkflow(c, id, "impact-pd", h.wfHandler.Submit, "SUBMIT")
}
...
func (h *Handler) Review(c *gin.Context) {
	...
	h.delegateWorkflow(c, id, "impact-pd", h.wfHandler.Review, "REVIEW")
}
...
func (h *Handler) Approve(c *gin.Context) {
	...
	h.delegateWorkflow(c, id, "impact-pd", h.wfHandler.Approve, "APPROVE")
}

// Approve2 handles POST /api/v1/master/impact-pd/:id/approve2 (ALCO step-up MFA).
func (h *Handler) Approve2(c *gin.Context) {
	id, ok := parseUUIDParam(c, "id")
	if !ok {
		return
	}
	h.delegateWorkflow(c, id, "impact-pd", h.wfHandler.Approve2, "APPROVE2")
}
...
func (h *Handler) Reject(c *gin.Context) {
	...
	h.delegateWorkflow(c, id, "impact-pd", h.wfHandler.Reject, "REJECT")
}
