...
// single-active invariant: no other APPROVED row for the same periode_id
// may already exist. If a duplicate is detected, BeforeCommit returns an error
...
func (h *WorkflowHook) BeforeCommit(ctx context.Context, tx *sql.Tx, evt workflow.HookEvent) error {
	...
	if err != nil {
		return fmt.Errorf("impactpd.WorkflowHook.BeforeCommit: load entity: %w", err)
	}
	tid := "TUGURE"
	if entity != nil {
		tid = entity.TenantID
	...
	if evt.NewState == workflow.StateApproved && entity != nil {
		count, cntErr := h.repo.CountDuplicateTx(ctx, tx, entity.PeriodeID, evt.EntityID, tid)
		if cntErr != nil {
			return fmt.Errorf("impactpd.WorkflowHook.BeforeCommit: count duplicate: %w", cntErr)
		}
		if count > 0 {
			return fmt.Errorf("impactpd.WorkflowHook.BeforeCommit: %w",
				domainerrors.New(domainerrors.CodeFLPeriodDuplicate,
					fmt.Sprintf("Sudah terdapat row APPROVED untuk periode %s. "+
						"Revoke atau hapus data lama sebelum approve.", entity.PeriodeID),
				),
			)
		}
	}
