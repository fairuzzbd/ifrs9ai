...
// worker_tasks_test.go — Tests for Worker task type and payload helpers.
...
//   - NewWorker panics on nil service or orchestrator.
...
func TestNewWorker_PanicOnNilService(t *testing.T) {
	...
	calcrun.NewWorker(nil, &eclcore.ECLOrchestrator{}, nil, nil)
}

// ─── Handle: bad JSON payload → returns error ─────────────────────────────────

func TestWorker_Handle_BadPayload(t *testing.T) {
...
func TestMax_Helper(t *testing.T) {
	...
	// This test documents the expected behavior without being able to call max directly.
	t.Log("max helper covered indirectly via Handle progressFn during computations")
}

func TestNewWorker_PanicOnNilOrchestrator(t *testing.T) {
	...
	// unexported, we verify the panic indirectly by calling NewWorker with
	// a non-nil service pointer obtained from a partial build.
	// The safest approach: the NewWorker guard runs orchestrator-nil check
	...
	calcrun.NewWorker(nil, nil, nil, nil) // triggers service-nil panic
}
