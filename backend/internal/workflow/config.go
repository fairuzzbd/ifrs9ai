...
// WorkflowConfig is the deserialized value of sys.config rows with key
...
func (c *WorkflowConfig) Validate() error {
	if c.Eyes != 4 && c.Eyes != 6 {
		return fmt.Errorf("WorkflowConfig.Eyes must be 4 or 6, got %d", c.Eyes)
	}
	if c.EntityType == "" {
		return fmt.Errorf("WorkflowConfig.EntityType must not be empty")
	}
	// DEC-017: these SoD rules cannot be disabled.
	if !c.SoDRules.ReviewerNotMaker {
		return fmt.Errorf("WorkflowConfig.SoDRules.ReviewerNotMaker must be true (DEC-017)")
	}
	if !c.SoDRules.ApproverNotMakerOrReviewer {
		return fmt.Errorf("WorkflowConfig.SoDRules.ApproverNotMakerOrReviewer must be true (DEC-017)")
	}
	return nil
}
...
// ConfigLoader abstracts reading WorkflowConfig rows from storage.
// Implementations: DBConfigLoader (production), InMemoryConfigLoader (tests).
type ConfigLoader interface {
	Load(entityType string) (*WorkflowConfig, error)
}
...
// Load returns the WorkflowConfig for entityType, using cache if fresh.
func (c *CachedConfigLoader) Load(entityType string) (*WorkflowConfig, error) {
...
func NewInMemoryConfigLoader(configs map[string]*WorkflowConfig) *InMemoryConfigLoader {
	upper := make(map[string]*WorkflowConfig, len(configs))
...
func (l *InMemoryConfigLoader) Load(entityType string) (*WorkflowConfig, error) {
	if cfg, ok := l.configs[strings.ToUpper(entityType)]; ok {
		return cfg, nil
	}
	return nil, fmt.Errorf("WorkflowConfig not found for entity type %q", entityType)
}

// ParseWorkflowConfig parses a JSON string into a WorkflowConfig and validates it.
func ParseWorkflowConfig(raw string) (*WorkflowConfig, error) {
	var cfg WorkflowConfig
	if err := json.Unmarshal([]byte(raw), &cfg); err != nil {
		return nil, fmt.Errorf("parse WorkflowConfig: %w", err)
	}
	if err := cfg.Validate(); err != nil {
		return nil, fmt.Errorf("validate WorkflowConfig: %w", err)
	}
	return &cfg, nil
}
...
func DefaultConfigs() map[string]*WorkflowConfig {
